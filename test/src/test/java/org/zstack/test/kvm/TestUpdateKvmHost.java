...
public class TestUpdateKvmHost {
    static CLogger logger = Utils.getLogger(TestUpdateKvmHost.class);
    ...
    public void test() throws ApiSenderException {
        ...
        inv.setUsername("admin");
        inv = api.updateKvmHost(inv, "admin");
        Assert.assertEquals("1", inv.getName());
        Assert.assertEquals("xxx", inv.getDescription());

        KVMHostVO vo = dbf.findByUuid(inv.getUuid(), KVMHostVO.class);
        Assert.assertEquals("admin", vo.getUsername());
